...
export function processEmployeeMonth(
  ...
  for (let day = 1; day <= daysInPeriod; day++) {
    ...
    const dayRoutePoints = routePoints.filter(p => {
      if (!p.timestamp) return false;
      const pDate = new Date(p.timestamp);
      return isSameDay(pDate, currentDate) || format(pDate, 'yyyy-MM-dd') === dateStr;
    });
    ...
    if (hasActivity) {
      ...
      // Do NOT auto-correct physical/net work hours for early departures; actual worked hours must be reflected
      if (approvedPermissionOnDay && !approvedPermissionOnDay.isAutoDeducted) {
        const permMinutes = getPermMinutesFromReq(approvedPermissionOnDay);
      ...
      // Phone pedometers record cumulative or periodic steps across events.
      // The highest step count recorded on any event during the day is the best representation of daily walking.
      const allStepValues = dayEvents
          .map(e => e.steps ?? 0)
          .filter(s => s > 0);
      let currentDaySteps = allStepValues.length > 0 ? Math.max(...allStepValues) : 0;

      // Deduct fake steps generated by phone pedometer during vehicle travel (>12km/h)
      // Standardized across all reports & dashboards: 100 fake steps per km of vehicle movement
      let vDist = (travelRes as any).vehicleDistance;
      if (!vDist && travelRes.distance > 3) {
          vDist = travelRes.distance - 3;
      }
      if (vDist && vDist > 0) {
          const fakeSteps = Math.min(Math.floor(currentDaySteps * 0.20), Math.floor(vDist * 25));
          currentDaySteps = Math.max(0, currentDaySteps - fakeSteps);
      }
