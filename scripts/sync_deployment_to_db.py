import openpyxl, json, re, urllib.request
from dotenv import dotenv_values

def main():
    print("Connecting to Supabase and loading Version_5.6 Final.xlsm...")
    ...
    # 1. Parse Excel Deployment sheet
    ...
    for c in range(2, 158):
        ...
        if name:
            ...
            total_val = float(tot or 0)
            clean_num = int(round(total_val))
            excel_sites[s_name] = clean_num

    print(f"Loaded {len(excel_sites)} sites from Excel Deployment sheet.")

    # 2. Fetch all organizations from Supabase
    ...
    print(f"Fetched {len(db_orgs)} organizations from Supabase.")
    ...
    ALIAS_MAP = {
        ...
        clean_key('42 Queens Square'): clean_key('42 ESTATES QUEENS SQUARE'),
        ...
        clean_key('Alanoville'): clean_key('ALANOVILLE- HARYAN & GOYAL CO'),
        ...
        clean_key('Nikoo Homes 4'): clean_key('NIKOO HOMES'),
        ...
        clean_key('Serene - Brigade Cornerstone Utopia'): clean_key('BRIGADE CORNERSTONE UTOPIA - SERENE'),
        ...
        clean_key('SNN Felicity'): clean_key('SNN ESTATE FELICITY'),
        clean_key('Uber Verdant Phase 2'): clean_key('UBER VERDANT PHASE 2'),
        clean_key('Uber Verdant Phase II'): clean_key('UBER VERDANT PHASE 2'),
        ...
        clean_key('Raja ritz Avenue Phase 1'): clean_key('RAJA RITZ AVENUE'),
        ...
        clean_key('Mahindra Lifespace Windchimes'): clean_key('MAHINDRA WINDCHIMES'),
        ...
        clean_key('Elita Prominade'): clean_key('ELITA PROMENADE'),
        ...
        clean_key('Paliwal (TTN)'): clean_key('PALIWAL (TITANIUM)'),
        clean_key('Wyzmindz'): clean_key('WYZMINDZ SOLUTIONS'),
        clean_key('GK ISPAT PVT LTD'): clean_key('GK ISPATS PVT LTD'),
        clean_key('Icon Sanctury'): clean_key('ICON SANCTUARY'),
        clean_key('NVT Under the Open Sky'): clean_key('NVT OPEN SKY'),
        clean_key('NVT Symphony of Orchid'): clean_key('NVT SYMPHONY OF ORCHIDS'),
    }
    ...
    for org in db_orgs:
        ...
        if target_clean and target_clean in excel_clean:
            orig_name, total_manpower = excel_clean[target_clean]
            
            # Send PATCH to update manpower_approved_count
            patch_data = json.dumps({'manpower_approved_count': total_manpower}).encode('utf-8')
            patch_req = urllib.request.Request(
                f"{url}/rest/v1/organizations?id=eq.{org['id']}",
                data=patch_data,
                headers={
                    'apikey': key,
                    'Authorization': f'Bearer {key}',
            ...
            try:
                with urllib.request.urlopen(patch_req) as p_resp:
                    updated_count += 1
                    old_cnt = org.get('manpower_approved_count')
                    updates_log.append(f"Updated {org['short_name']} ({org['id']}): {old_cnt} -> {total_manpower} (Excel: {orig_name})")
            except Exception as e:
                print(f"Failed to update {org['id']}: {e}")

    print(f"\nSuccessfully updated {updated_count} organizations in Supabase!")
    print("\nSample updates:")
    for log in updates_log[:30]:
        print(" ", log)
