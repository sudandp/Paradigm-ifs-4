...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const path = (req.query.path as string) || '/camera/snapshot/main_gate_entry';
  ...
  for (const base of candidateBases) {
    const targetUrl = `${base}${path.startsWith('/') ? '' : '/'}${path}${req.url?.includes('?') ? req.url.slice(req.url.indexOf('?')) : ''}`;
    try {
