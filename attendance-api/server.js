/**
 * ═══════════════════════════════════════════════════════════════
 *  Paradigm FMS — Attendance API Proxy
...
// ─── MS SQL Connection (connects to localhost — never internet) ─
const DB_CONFIG = {
  server: 'localhost',
...
app.get('/attendance', requireApiKey, async (req, res) => {
  ...
  try {
    ...
    // ── Build 7-Day Attendance Trend (Multi-Month UNION ALL Engine) ──────────
    ...
    console.log(`[API] ✅ Success: Total=${totalEmployees}, Present=${present}, Absent=${absent}, TrendItems=${trend.length}`);
...
// ─── GET /tables — helper to discover your DB schema ──────────
...
// ─── GET /devices — biometric device status ────────────────────
...
// ─── GET /diagnose — full data health check for today ─────────
...
// ─── GET /columns/:table — get columns of a specific table ────
