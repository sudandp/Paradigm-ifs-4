  ...
  return (
    <div className={`animate-fade-in min-w-0 overflow-x-hidden min-h-screen ${isMobile ? 'bg-[#041b0f] text-white p-4 pt-6 space-y-6 pb-24' : 'space-y-8 pb-32 md:pb-8'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 min-h-[calc(100vh-12rem)]">
        {/* 1. Templates list */}
        <div className={`lg:col-span-1 flex flex-col overflow-hidden ${isMobile ? 'bg-[#092c19] rounded-[24px] border border-[#134426] p-5 shadow-[0_4px_16px_rgba(0,0,0,0.35)] relative' : 'bg-white rounded-3xl border border-border p-5 shadow-sm'}`}>
          ...
          <div className="flex items-center gap-3 mb-6 flex-shrink-0">
            <div className={`w-10 h-10 rounded-2xl flex items-center justify-center ${isMobile ? 'bg-[#041b0f] border border-[#134426] text-[#44D62C]' : 'bg-accent/5 text-accent'}`}>
              ...
              <button
                ...
                className={`w-full text-left p-4 rounded-2xl transition-all flex flex-col gap-1.5 group border ${
                  selectedTemplate?.id === temp.id
                    ? (isMobile ? 'border-[#44D62C]/50 bg-[#041b0f] shadow-lg shadow-black/30' : 'border-accent/40 bg-accent/5 shadow-md')
                    : (isMobile ? 'border-transparent hover:bg-[#041b0f]/50' : 'border-transparent hover:bg-page hover:border-border')
                }`}
              >
                <span className={`text-sm font-bold leading-tight ${isMobile ? 'text-white group-hover:text-[#44D62C]' : 'text-primary-text group-hover:text-accent'} transition-colors`}>{temp.name}</span>
                <span className={`text-[10px] font-bold uppercase tracking-widest ${isMobile ? 'text-[#7D967B]' : 'text-muted'}`}>{temp.letterType}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className={`lg:col-span-2 flex flex-col overflow-hidden ${isMobile ? 'bg-[#092c19] rounded-[24px] border border-[#134426] shadow-[0_4px_16px_rgba(0,0,0,0.35)] relative' : 'bg-white rounded-3xl border border-border shadow-sm'}`}>
          {isMobile && <div className="absolute inset-x-0 top-0 h-[1px] bg-gradient-to-r from-transparent via-[#44D62C]/25 to-transparent pointer-events-none" />}
          {/* Workspace toolbar */}
          <div className={`p-5 flex items-center justify-between flex-shrink-0 border-b ${isMobile ? 'border-[#134426] bg-[#041b0f]' : 'border-border bg-page/50'}`}>
            <div>
              <h3 className={`text-sm font-bold ${isMobile ? 'text-white' : 'text-primary-text'}`}>
                {selectedTemplate?.name || 'Select a template'}
              </h3>
              <p className={`text-[10px] font-bold uppercase tracking-widest mt-0.5 ${isMobile ? 'text-[#7D967B]' : 'text-muted'}`}>
                HTML Document Definition
              </p>
            </div>

            <div className={`flex p-1 rounded-2xl border ${isMobile ? 'bg-[#072414] border-[#134426]' : 'bg-page border-border'}`}>
              <button
          ...
          {/* Editor content */}
          {mode === 'edit' ? (
            <div className={`flex-1 p-4 overflow-hidden flex flex-col ${isMobile ? 'bg-[#041b0f]' : 'bg-page/30'}`}>
              <textarea
                value={bodyHtml}
                onChange={(e) => setBodyHtml(e.target.value)}
                className={`flex-1 w-full p-5 rounded-2xl font-mono text-xs leading-relaxed outline-none resize-none transition-all ${isMobile ? 'bg-[#072414] border border-[#134426] text-white/90 placeholder:text-[#7D967B] focus:border-[#44D62C]' : 'bg-white border border-border text-primary-text shadow-inner focus:ring-2 focus:ring-accent/10'}`}
              />
            </div>
          ) : (
            <div className={`flex-1 p-6 overflow-y-auto flex flex-col items-center ${isMobile ? 'bg-[#041b0f]' : 'bg-slate-100'}`}>
        ...
        <div className={`lg:col-span-1 flex flex-col overflow-hidden ${isMobile ? 'bg-[#092c19] rounded-[24px] border border-[#134426] p-5 shadow-[0_4px_16px_rgba(0,0,0,0.35)] relative' : 'bg-white rounded-3xl border border-border p-5 shadow-sm'}`}>
          {isMobile && <div className="absolute inset-x-0 top-0 h-[1px] bg-gradient-to-r from-transparent via-[#44D62C]/25 to-transparent pointer-events-none" />}
          <div className="flex items-center gap-3 mb-6 flex-shrink-0">
            <div className={`w-10 h-10 rounded-2xl flex items-center justify-center ${isMobile ? 'bg-[#041b0f] border border-[#134426] text-[#44D62C]' : 'bg-blue-500/10 text-blue-600'}`}>
          ...
          {/* Hint banner */}
          <div className={`flex items-start gap-2 p-3 rounded-2xl mb-4 flex-shrink-0 ${isMobile ? 'bg-[#041b0f] border border-[#134426]' : 'bg-amber-50 border border-amber-200'}`}>
          ...
          <div className="flex-1 overflow-y-auto space-y-2.5 pr-1 custom-scrollbar">
            {PLACEHOLDERS.map((ph) => (
              <div
                ...
                className={`p-3.5 rounded-2xl cursor-pointer group transition-all border ${isMobile ? 'border-transparent hover:border-[#134426] hover:bg-[#041b0f]' : 'border-transparent hover:border-accent/30 hover:bg-accent/5 hover:shadow-sm'}`}
              >
                <div className="flex justify-between items-center mb-1">
                  <code className={`text-xs font-black transition-colors ${isMobile ? 'text-white group-hover:text-[#44D62C]' : 'text-primary-text group-hover:text-accent'}`}>
                    {ph.token}
                  </code>
                  <Copy className={`w-3.5 h-3.5 opacity-0 group-hover:opacity-100 transition-all ${isMobile ? 'text-[#44D62C]' : 'text-accent'}`} />
                </div>
                <p className={`text-[10px] leading-normal ${isMobile ? 'text-[#7D967B]' : 'text-muted'}`}>
                  {ph.desc}
                </p>
              </div>
            ))}
