          ...
          <View style={{ marginTop: 15, flexDirection: 'row', justifyContent: 'space-between', borderTopWidth: 1, borderTopColor: '#F1F5F9', paddingTop: 8 }}>
            <View style={{ flexDirection: 'row', gap: 15 }}>
              ...
              <Text style={{ fontSize: 7, color: '#94A3B8' }}>
                GENERATED BY: {generatedBy?.toUpperCase() || 'SYSTEM'} 
                {generatedByRole ? ` (${generatedByRole.toUpperCase()})` : ''}
              </Text>
            </View>
...
export interface SiteOtDataRow {
  userName: string;
  date: string;
  siteOtIn: string | null;
  siteOtOut: string | null;
  duration: string | null;
  locationName?: string;
}

export const SiteOtReportDocument: React.FC<{ 
  data: SiteOtDataRow[]; 
  dateRange: { startDate: Date; endDate: Date }; 
  generatedBy?: string;
  generatedByRole?: string;
        ...
        <Page key={pageIndex} size="A4" orientation="landscape" style={styles.page}>
          ...
          <View style={styles.table}>
            <View style={[styles.tableRow, { backgroundColor: '#f2f2f2' }]}>
              ...
              <View style={[styles.tableColHeader, { width: '15%' }]}><Text style={styles.tableCellHeader}>Site OT Out</Text></View>
              <View style={[styles.tableColHeader, { width: '15%' }]}><Text style={styles.tableCellHeader}>Duration</Text></View>
            ...
            {pageData.map((row, idx) => (
              <View key={idx} style={[styles.tableRow, { backgroundColor: idx % 2 === 0 ? '#fff' : '#fafafa' }]}>
                ...
                <View style={[styles.tableCol, { width: '15%' }]}><Text style={styles.tableCell}>{row.siteOtOut || '-'}</Text></View>
                <View style={[styles.tableCol, { width: '15%' }]}><Text style={styles.tableCell}>{row.duration || '-'}</Text></View>
                <View style={[styles.tableCol, { width: '15%' }]}><Text style={styles.tableCell}>{row.locationName || '-'}</Text></View>
              </View>
            ))}
          </View>
...
export interface AuditLogDataRow {
  dateTime: string;
  action: string;
...
export const AuditLogDocument: React.FC<{ 
  data: AuditLogDataRow[]; 
  dateRange: { startDate: Date; endDate: Date }; 
  generatedBy?: string;
  generatedByRole?: string;
  targetUserName?: string;
        ...
        <Page key={pageIndex} size="A4" orientation="landscape" style={styles.page}>
          ...
          <View style={styles.table}>
            <View style={[styles.tableRow, { backgroundColor: '#f2f2f2' }]}>
              ...
              <View style={[styles.tableColHeader, { width: '20%' }]}><Text style={styles.tableCellHeader}>Target Employee</Text></View>
            ...
            {pageData.map((row, idx) => (
              <View key={idx} style={[styles.tableRow, { backgroundColor: idx % 2 === 0 ? '#fff' : '#fafafa' }]}>
                ...
                <View style={[styles.tableCol, { width: '20%' }]}><Text style={styles.tableCell}>{row.target_name}</Text></View>
                <View style={[styles.tableCol, { width: '30%' }]}><Text style={styles.tableCell}>{row.detailsStr}</Text></View>
              </View>
            ))}
          </View>

          <Text style={styles.footer} render={({ pageNumber, totalPages }) => (
            `Paradigm Services - Audit Report - Page ${pageNumber} of ${totalPages}`
          )} fixed />
        </Page>
      ))}
    </Document>
...
// ── Detailed Audit 31-Day Attendance Report PDF Document (Matching Image 2 Layout) ──

export interface DetailedAuditPdfDataRow {
  dayNum: number;
  status: string;
  inTime: string;
  outTime: string;
...
export interface DetailedAuditPdfEmployee {
  empCode: string;
  empName: string;
  designation: string;
  department: string;
  ...
  shiftGsCount: number;
  shiftNsCount: number;
  dailyData: DetailedAuditPdfDataRow[];
}
...
export const DetailedAuditPdfDocument: React.FC<{
  ...
  generatedBy?: string;
  periodLabel?: string;
}> = ({ employees, generatedBy, periodLabel }) => {
        ...
        <Page key={`${emp.empCode}-${empIdx}`} size="A4" orientation="landscape" style={matrixStyles.page}>
          ...
          <View style={matrixStyles.cardContainer}>
            ...
            <View style={matrixStyles.kpiRow}>
              <View style={[matrixStyles.kpiBox, { backgroundColor: '#f0fdf4', borderColor: '#bbf7d0' }]}>
                <Text style={[matrixStyles.kpiLabel, { color: '#166534' }]}>NET WORK</Text>
                <Text style={[matrixStyles.kpiVal, { color: '#15803d' }]}>{emp.netWorkHrs} Hrs</Text>
              </View>
              <View style={[matrixStyles.kpiBox, { backgroundColor: '#ecfdf5', borderColor: '#a7f3d0' }]}>
                <Text style={[matrixStyles.kpiLabel, { color: '#047857' }]}>TOTAL OT</Text>
                <Text style={[matrixStyles.kpiVal, { color: '#059669' }]}>{emp.totalOtHrs} Hrs</Text>
              </View>
              <View style={[matrixStyles.kpiBox, { backgroundColor: '#fefce8', borderColor: '#fef08a' }]}>
                <Text style={[matrixStyles.kpiLabel, { color: '#854d0e' }]}>AVG HRS/DAY</Text>
                <Text style={[matrixStyles.kpiVal, { color: '#a16207' }]}>{emp.avgHrsPerDay} Hrs</Text>
              </View>
              <View style={matrixStyles.kpiBox}>
                <Text style={matrixStyles.kpiLabel}>GROSS / BREAK</Text>
                <Text style={matrixStyles.kpiVal}>GROSS: {emp.grossHrs}h / BREAK: {emp.breakHrs}h</Text>
              </View>
              <View style={[matrixStyles.kpiBox, { backgroundColor: '#f8fafc' }]}>
                <Text style={matrixStyles.kpiLabel}>ATTENDANCE DISTRIBUTION</Text>
                <Text style={{ fontSize: 7, color: '#334155', marginTop: 2 }}>
                  Paid: <Text style={{ fontWeight: 'bold', color: '#16a34a' }}>{emp.paidDays}</Text> | Absent: <Text style={{ fontWeight: 'bold', color: '#dc2626' }}>{emp.absentDays}</Text> | W/O: <Text style={{ fontWeight: 'bold' }}>{emp.weeklyOffs}</Text> | Payable: <Text style={{ fontWeight: 'bold', color: '#006B3F' }}>{emp.payableDays}</Text>
                </Text>
              </View>
            </View>

            {/* 31-Day Matrix Grid Table */}
            <View style={matrixStyles.gridTable}>
              {/* Header Row: Date */}
              <View style={[matrixStyles.gridRow, { backgroundColor: '#f8fafc' }]}>
                <Text style={matrixStyles.labelCell}>Date</Text>
                {daysArray.map(d => (
                  <Text key={d} style={[matrixStyles.dayCell, { fontWeight: 'bold' }]}>{d}</Text>
                ))}
              </View>
            ...
            {/* Bottom Stats Bar */}
            <View style={matrixStyles.statsBar}>
              <Text>AVG WORKING HOURS: {emp.avgHrsPerDay}H</Text>
              <Text>SITE PRESENCE SCORE: {emp.presenceScorePct}%</Text>
              <Text>SHIFT DISTRIBUTION: Shift GS({emp.shiftGsCount}) Shift NS({emp.shiftNsCount})</Text>
            </View>

            {/* Notation Reference Footer */}
            <View style={matrixStyles.legendRow}>
              <Text style={[matrixStyles.legendBadge, { backgroundColor: '#dcfce7', color: '#15803d' }]}>P Present</Text>
              <Text style={[matrixStyles.legendBadge, { backgroundColor: '#e0e7ff', color: '#4338ca' }]}>0.5P Half Day</Text>
              <Text style={[matrixStyles.legendBadge, { backgroundColor: '#ccfbf1', color: '#0f766e' }]}>0.75P Three Quarter Day</Text>
              <Text style={[matrixStyles.legendBadge, { backgroundColor: '#fee2e2', color: '#b91c1c' }]}>A Absent</Text>
              <Text style={[matrixStyles.legendBadge, { backgroundColor: '#ffedd5', color: '#c2410c' }]}>LOP Loss of Pay</Text>
              <Text style={[matrixStyles.legendBadge, { backgroundColor: '#e0f2fe', color: '#0369a1' }]}>W/O Weekly Off</Text>
              <Text style={[matrixStyles.legendBadge, { backgroundColor: '#dbeafe', color: '#1d4ed8' }]}>H Public Holiday</Text>
