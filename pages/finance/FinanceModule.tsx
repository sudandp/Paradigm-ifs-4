...
const FinanceModule: React.FC = () => {
    ...
    // Director check: Director only sees Attendance Tracker, NOT Monthly Invoice Tracker (finance data)
    ...
    const rawTab = queryParams.get('tab') || 'attendance';
    const initialTab = (isDirector && rawTab === 'site') ? 'attendance' : rawTab;
    const [activeTab, setActiveTab] = useState<string>(initialTab);

    useEffect(() => {
        const tab = queryParams.get('tab') || 'attendance';
        if (isDirector && tab === 'site') {
    ...
    return (
        <div className="flex flex-col min-h-full">
            <div className="p-4 md:p-6 space-y-4 md:space-y-6 flex-1">
                ...
                <div className={!isDirector ? "mt-4 md:mt-6" : ""}>
                    {activeTab === 'attendance' || isDirector ? <SiteAttendanceTracker /> : <SiteFinanceTracker />}
                </div>
            </div>
        </div>
    );
