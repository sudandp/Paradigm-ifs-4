...
async function testFinalSave() {
    console.log('Testing final saveEntity logic with valid company ID...');
    
    const entityId = `ent_test_${Date.now()}`;
    const testEntity = {
        id: entityId,
        name: 'Final Test Entity Storage SUCCESS',
    ...
    if (error) {
        ...
        if (data.status === 'completed' && data.epfo_doc_url === 'https://test.url/final_epfo.pdf') {
            console.log('✅ ALL TESTS PASSED: Data persisted correctly.');
        } else {
            console.log('❌ SOME FIELDS MISSING: Data persistence failure.');
        }
