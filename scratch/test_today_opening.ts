...
async function setTodayOpening() {
    const userId = '216d264d-b20b-45a1-91b5-09cb47c781fb'; // Veerabhadra T M

    // Set opening date to today 2026-07-23 with balance 3.5
    await supabase.from('users').update({
        comp_off_opening_balance: 3.5,
        comp_off_opening_date: '2026-07-23',
        updated_at: new Date().toISOString()
    }).eq('id', userId);

    console.log("Set comp_off_opening_balance = 3.5, comp_off_opening_date = '2026-07-23'");
}

setTodayOpening().catch(console.error);
