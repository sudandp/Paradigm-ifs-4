...
async function verifyAll() {
  console.log('=== VERIFYING DATABASE STABILITY & HARDENING ===');
  ...
  for (const table of tables) {
    ...
    if (error) {
      ...
      passed++;
      console.log(`✅ Table ${table}: Ready (${data?.length ?? 0} rows inspected)`);
    }
  }

  console.log(`\nResults: ${passed}/${tables.length} tables verified healthy.`);
}
