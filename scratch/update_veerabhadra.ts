...
async function updateVeerabhadra() {
    const userId = '216d264d-b20b-45a1-91b5-09cb47c781fb'; // Veerabhadra T M

    console.log("Updating Veerabhadra T M in database...");

    const { data, error } = await supabase
        .from('users')
        .update({
            comp_off_opening_balance: 18,
            comp_off_opening_date: '2026-01-01',
    ...
    console.log("Successfully updated Veerabhadra T M in database!");
    console.log(data);
}

updateVeerabhadra().catch(console.error);
