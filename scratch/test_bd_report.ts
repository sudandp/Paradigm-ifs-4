...
async function run() {
  ...
  // Test date: 16th July 2026
  const targetDateStr = '2026-07-16T18:00:00+05:30';
  const nowIST = new Date(targetDateStr);
  const nowUTC = new Date(nowIST.getTime() - IST_OFFSET);
  const todayStr = getISTDateString(nowIST);
  const startOfTodayUTC = startOfDay(nowUTC);

  console.log('Testing for date:', targetDateStr);
  console.log('todayStr:', todayStr);
  console.log('startOfTodayUTC:', startOfTodayUTC.toISOString());

  // Copying logic from crm_bd_daily
  const { data: usersRes } = await supabase.from('users').select('id, name, role:roles(display_name)');
  ...
  const [eventsRes, leadsRes, callsRes] = await Promise.all([
    supabase.from('attendance_events').select('user_id, type, timestamp, latitude, longitude, travel_distance').gte('timestamp', startOfTodayUTC.toISOString()).lt('timestamp', new Date(startOfTodayUTC.getTime() + 24*60*60*1000).toISOString()).order('timestamp', { ascending: true }),
    supabase.from('crm_leads').select('id, created_by, assigned_to, company_name, contact_person, status, created_at').gte('created_at', startOfTodayUTC.toISOString()).lt('created_at', new Date(startOfTodayUTC.getTime() + 24*60*60*1000).toISOString()),
    supabase.from('crm_followups').select('created_by, type, lead_id, created_at').gte('created_at', startOfTodayUTC.toISOString()).lt('created_at', new Date(startOfTodayUTC.getTime() + 24*60*60*1000).toISOString())
  ]);
  ...
  for (const bd of bdUsers) {
    ...
    // New logic to calculate prospect calls vs followup calls
    ...
    console.log('Distance:', kms_travelled);
    console.log('Event Points:', bdEvents.map((e: any) => ({ type: e.type, lat: e.latitude, lon: e.longitude })));
    ...
    console.log('Working hours for', bd.name, ':', working_hours, check_in_time, check_out_time);
    reports.push({
      bd_name: bd.name || 'BD',
  ...
  if (!rules || rules.length === 0) {
      console.log('No active schedules for crm_bd_daily');
      return;
  }
  ...
  const transporter = nodemailer.createTransport({
    host: emailConfig.host || 'smtp.gmail.com', port: emailConfig.port || 587, secure: emailConfig.secure || false,
  ...
  for (const dataItem of reports) {
    let greetingMessage = `Here is the Daily Activity Report for {bd_name}. The data below reflects activities logged on {report_date}.`;
    ...
    try {
      ...
      console.log('Mail sent successfully for', dataItem.bd_name);
    } catch (mailErr: any) {
