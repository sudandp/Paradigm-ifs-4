...
const Logo: React.FC<{ className?: string; localPath?: string }> = ({ className = '', localPath }) => {
    ...
    return (
        <img
            src={src}
            alt="Paradigm Logo"
            className={`w-auto object-contain filter drop-shadow-[0_2px_8px_rgba(34,197,94,0.3)] brightness-110 transition-all duration-500 ${!className.includes('h-') && 'h-10'} ${className}`}
            onError={(e) => {
